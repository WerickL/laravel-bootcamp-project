    ...
    <div class="p-6 flex space-x-2">
        ...
        <div class="flex-1">
            ...
            <form v-if="editing" @submit.prevent="form.put(route('tasks.update', task.id), { onSuccess: () => editing = false })">
                <textarea v-model="form.description" class="mt-4 w-full text-gray-900 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm"></textarea>
            ...
            <p v-else class="mt-4 text-lg text-gray-900">{{ task.description }}</p>
        </div>
    </div>
</template>